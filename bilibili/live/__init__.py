...
@scheduler.scheduled_job('cron', second='*/30', id='live_sched', misfire_grace_time=10, coalesce=True, max_instances=5)
async def living():
...
async def add(bot: Bot, event: Event, state: T_State):
    ...
    try:
        mid = (str(event.get_message())).split()
        del mid[0]
        if 'uid' in mid[0]:
            mid = mid[0].split(':')[-1]
            mid = int(mid)
...
async def add(bot: Bot, event):
    ...
    # try:
    key = str(event.get_message()).split()
    key = key[1]
    print(key)
    if key.isdigit():
